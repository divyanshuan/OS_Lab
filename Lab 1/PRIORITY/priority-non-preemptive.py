class Priority:
    def processData(self, no_of_processes):
        ...
        for i in range(no_of_processes):
            temporary = []
            process_id,arrival_time,burst_time, priority = map(int,input("Enter Process ID, Arrival Time, Burst Time, Priority: ").split())
    ...
    def calculateWaitingTime(self, process_data):
        ...
        for i in range(len(process_data)):
            waiting_time = process_data[i][6] - process_data[i][1]-process_data[i][5]  # waiting_time = turnaround_time - burst_time
